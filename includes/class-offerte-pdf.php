...
class Offerte_PDF {
    ...
    private function init_dompdf() {
        // Load DOMPDF — try Boost Calculator's bundled library first.
        if ( ! class_exists( 'Dompdf\\Dompdf' ) ) {
            $bc_dir = \BoostOfferte\get_boost_calculator_dir();
            if ( $bc_dir && file_exists( $bc_dir . 'includes/pdf/autoload.php' ) ) {
                require_once $bc_dir . 'includes/pdf/autoload.php';
            } elseif ( file_exists( BOOST_OFFERTE_PLUGIN_DIR . 'vendor/autoload.php' ) ) {
                require_once BOOST_OFFERTE_PLUGIN_DIR . 'vendor/autoload.php';
            }
        }

        if ( ! class_exists( 'Dompdf\\Dompdf' ) ) {
            wp_die( esc_html__( 'DOMPDF is niet beschikbaar. Zorg dat Boost Calculator actief is of installeer DOMPDF via Composer.', 'boost-offerte' ) );
        }
    ...
    private function render_template() {
        ...
        ob_start();
        include BOOST_OFFERTE_PLUGIN_DIR . 'templates/pdf/offerte.php';
        return ob_get_clean();
    }
