...
class Offerte_Ajax {
    ...
    public function ajax_save_offerte() {
        ...
        if ( ! current_user_can( 'manage_woocommerce' ) ) {
            wp_send_json_error( array( 'message' => __( 'Geen toegang.', 'boost-offerte' ) ) );
        }
        ...
        if ( $offerte_id ) {
            ...
            if ( ! $offerte->is_valid() ) {
                wp_send_json_error( array( 'message' => __( 'Offerte niet gevonden.', 'boost-offerte' ) ) );
            }
        } else {
            $offerte = Offerte_Model::create( $data );
            if ( ! $offerte ) {
                wp_send_json_error( array( 'message' => __( 'Offerte kon niet worden aangemaakt.', 'boost-offerte' ) ) );
            }
        }
        ...
        wp_send_json_success( array(
            'message'    => __( 'Offerte opgeslagen.', 'boost-offerte' ),
    ...
    public function ajax_send_offerte() {
        ...
        if ( ! $pdf_path ) {
            wp_send_json_error( array( 'message' => __( 'PDF kon niet worden gegenereerd.', 'boost-offerte' ) ) );
        }
        ...
        if ( ! $success ) {
            wp_send_json_error( array( 'message' => __( 'E-mail kon niet worden verzonden.', 'boost-offerte' ) ) );
        }
        ...
        wp_send_json_success( array(
            'message' => __( 'Offerte is verzonden.', 'boost-offerte' ),
    ...
    public function ajax_calc_line_item() {
        ...
        if ( ! $calculator_id ) {
            wp_send_json_error( array( 'message' => __( 'Geen calculator opgegeven.', 'boost-offerte' ) ) );
        }

        if ( ! Offerte_Settings::has_calculator_integration() ) {
            wp_send_json_error( array( 'message' => __( 'Boost Calculator is niet beschikbaar.', 'boost-offerte' ) ) );
        }

        $result = \Bossier\Calculator\Price_Calculator::calculate_from_request( $calculator_id, $selections, $product_id );

        if ( false === $result ) {
            wp_send_json_error( array( 'message' => __( 'Berekening mislukt.', 'boost-offerte' ) ) );
        }
    ...
    public function ajax_get_calc_fields() {
        ...
        if ( ! $calculator_id ) {
            wp_send_json_error( array( 'message' => __( 'Geen calculator ID.', 'boost-offerte' ) ) );
        }
        ...
        if ( ! $calculator->is_valid() ) {
            wp_send_json_error( array( 'message' => __( 'Calculator niet gevonden.', 'boost-offerte' ) ) );
        }
    ...
    public function ajax_generate_pdf() {
        ...
        wp_send_json_success( array(
            'message'  => __( 'PDF gegenereerd.', 'boost-offerte' ),
    ...
    public function ajax_cancel_offerte() {
        ...
        wp_send_json_success( array(
            'message' => __( 'Offerte is geannuleerd.', 'boost-offerte' ),
        ) );
    }
    ...
    public function ajax_duplicate_offerte() {
        ...
        if ( ! $new_offerte ) {
            wp_send_json_error( array( 'message' => __( 'Dupliceren mislukt.', 'boost-offerte' ) ) );
        }

        wp_send_json_success( array(
            'message'    => __( 'Offerte gedupliceerd.', 'boost-offerte' ),
    ...
    public function ajax_accept_offerte() {
        ...
        if ( ! hash_equals( $offerte->get_access_token(), $token ) ) {
            wp_send_json_error( array( 'message' => __( 'Ongeldige toegang.', 'boost-offerte' ) ) );
        }

        // Verify nonce.
        if ( ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ?? '' ) ), 'bs_offerte_accept_' . $offerte_id ) ) {
            wp_send_json_error( array( 'message' => __( 'Beveiligingscontrole mislukt.', 'boost-offerte' ) ) );
        }

        // Check if offerte can be accepted.
        if ( ! $offerte->can_accept() ) {
            wp_send_json_error( array( 'message' => __( 'Deze offerte kan niet meer worden geaccepteerd.', 'boost-offerte' ) ) );
        }

        // Save signature.
        $signature_data = ! empty( $_POST['signature'] ) ? $_POST['signature'] : '';
        if ( empty( $signature_data ) ) {
            wp_send_json_error( array( 'message' => __( 'Handtekening is vereist.', 'boost-offerte' ) ) );
        }

        $signature_path = Offerte_Signature::save_signature( $signature_data, $offerte->get_quote_number() );
        if ( ! $signature_path ) {
            wp_send_json_error( array( 'message' => __( 'Handtekening kon niet worden opgeslagen.', 'boost-offerte' ) ) );
        }
        ...
        $response = array(
            'message' => __( 'Offerte geaccepteerd! Uw bestelling wordt verwerkt.', 'boost-offerte' ),
        );
    ...
    public function ajax_download_pdf() {
        ...
        if ( ! $offerte->is_valid() ) {
            wp_die( esc_html__( 'Offerte niet gevonden.', 'boost-offerte' ), '', array( 'response' => 404 ) );
        }

        if ( ! hash_equals( $offerte->get_access_token(), $token ) ) {
            wp_die( esc_html__( 'Ongeldige toegang.', 'boost-offerte' ), '', array( 'response' => 403 ) );
        }
