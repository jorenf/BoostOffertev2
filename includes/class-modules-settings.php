...
class Modules_Settings {
    ...
    public function sanitize_settings( $input ) {
        ...
        // Offerte settings (preserve existing if not in form)
        $offerte_text_fields = array(
            'offerte_prefix', 'offerte_sender_name', 'offerte_sender_email',
            'offerte_company_name', 'offerte_company_address', 'offerte_company_phone',
            'offerte_company_kvk', 'offerte_company_btw', 'offerte_company_iban',
            'offerte_terms_text',
        );
        foreach ( $offerte_text_fields as $field ) {
            if ( isset( $input[ $field ] ) ) {
                $sanitized[ $field ] = 'offerte_sender_email' === $field
        ...
        $offerte_textarea_fields = array(
            'offerte_email_sent_subject', 'offerte_email_sent_body',
            'offerte_email_reminder_subject', 'offerte_email_reminder_body',
        );
        foreach ( $offerte_textarea_fields as $field ) {
            $sanitized[ $field ] = isset( $input[ $field ] ) ? sanitize_textarea_field( $input[ $field ] ) : ( $existing[ $field ] ?? '' );
        }

        $offerte_numeric_fields = array( 'offerte_default_validity', 'offerte_default_reminder' );
        foreach ( $offerte_numeric_fields as $field ) {
            $sanitized[ $field ] = isset( $input[ $field ] ) ? absint( $input[ $field ] ) : ( $existing[ $field ] ?? 0 );
        }

        $sanitized['offerte_company_logo'] = isset( $input['offerte_company_logo'] ) ? absint( $input['offerte_company_logo'] ) : ( $existing['offerte_company_logo'] ?? '' );
        $sanitized['offerte_terms_url']    = isset( $input['offerte_terms_url'] ) ? esc_url_raw( $input['offerte_terms_url'] ) : ( $existing['offerte_terms_url'] ?? '' );

        if ( isset( $input['offerte_payment_methods'] ) && is_array( $input['offerte_payment_methods'] ) ) {
            $sanitized['offerte_payment_methods'] = array_map( 'sanitize_key', $input['offerte_payment_methods'] );
        } else {
            $sanitized['offerte_payment_methods'] = $existing['offerte_payment_methods'] ?? array( 'ideal_invoice' );
        }
    ...
    public function render_settings_page() {
        ...
        $tabs = array(
            ...
            'offerte'  => __( 'Offertes', 'bossier-calculator' ),
        );
    ...
    private function is_field_in_current_tab( $field, $input ) {
        ...
        // Offerte tab fields
        $offerte_fields = array(
            'offerte_module_enabled',
        );
        if ( in_array( $field, $offerte_fields, true ) ) {
            return isset( $input['offerte_prefix'] ) || isset( $input['offerte_sender_name'] );
        }
    ...
    /**
     * Check if Offerte module is enabled.
     *
     * @return bool
     */
    public static function is_offerte_enabled() {
        $settings = self::get_settings();
        return ! empty( $settings['offerte_module_enabled'] );
    }
