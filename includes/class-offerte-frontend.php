...
class Offerte_Frontend {
    ...
    public function handle_template( $template ) {
        ...
        if ( empty( $token ) || ! hash_equals( $offerte->get_access_token(), $token ) ) {
            wp_die(
                esc_html__( 'Ongeldige toegangslink. Controleer de link in uw e-mail.', 'boost-offerte' ),
                esc_html__( 'Toegang geweigerd', 'boost-offerte' ),
                array( 'response' => 403 )
            );
        }
        ...
        if ( 'offerte-accepted' === $offerte->get_status() ) {
            return BOOST_OFFERTE_PLUGIN_DIR . 'frontend/views/offerte-accepted.php';
        }

        return BOOST_OFFERTE_PLUGIN_DIR . 'frontend/views/offerte-public.php';
    }
    ...
    public function enqueue_frontend_assets() {
        ...
        wp_enqueue_style(
            'bs-offerte-frontend',
            BOOST_OFFERTE_PLUGIN_URL . 'assets/css/offerte-frontend.css',
            array(),
        ...
        wp_enqueue_script(
            'signature-pad',
            BOOST_OFFERTE_PLUGIN_URL . 'assets/js/vendor/signature_pad.umd.min.js',
            array(),
            '4.1.7',
            true
        );
        ...
        wp_enqueue_script(
            'bs-offerte-frontend',
            BOOST_OFFERTE_PLUGIN_URL . 'assets/js/offerte-frontend.js',
        ...
        wp_localize_script( 'bs-offerte-frontend', 'bsOfferte', array(
            ...
            'i18n'     => array(
                'signatureRequired' => __( 'Plaats uw handtekening a.u.b.', 'boost-offerte' ),
                'termsRequired'     => __( 'U dient akkoord te gaan met de voorwaarden.', 'boost-offerte' ),
                'processing'        => __( 'Bezig met verwerken...', 'boost-offerte' ),
                'error'             => __( 'Er is een fout opgetreden. Probeer het opnieuw.', 'boost-offerte' ),
            ),
        ) );
    }
}
