...
class Offerte_Order_Creator {
    ...
    public function create_order() {
        ...
        $order->add_order_note(
            sprintf(
                ...
                __( 'Aangemaakt vanuit offerte %1$s, digitaal ondertekend op %2$s', 'boost-offerte' ),
    ...
    private function add_line_items( \WC_Order $order ) {
        ...
        foreach ( $items as $item_data ) {
            ...
            if ( $product_id && ( $product = wc_get_product( $product_id ) ) ) {
                ...
                $item->set_name( ! empty( $item_data['title'] ) ? $item_data['title'] : __( 'Product', 'boost-offerte' ) );
    ...
    private function add_shipping( \WC_Order $order ) {
        ...
        if ( $shipping_cost > 0 ) {
            ...
            $shipping->set_method_title( __( 'Verzending', 'boost-offerte' ) );
